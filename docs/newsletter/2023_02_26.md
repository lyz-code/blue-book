...
* Correction: [Update the leader key section.](vim.md#the-leader-key)
    ...
    * Use the keys directly instead of using `<leader>`
    ...
    Defining `mapleader` and/or using `<leader>` may be useful if you change your mind often on what key to use a leader but it won't be of any use if your mappings are stable.[![](not-by-ai.svg){: .center}](https://notbyai.fyi)
