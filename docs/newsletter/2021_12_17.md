...
* Correction: Warn against upper version pinning.
    ...
    The main problem is that `poetry add` does [upper pinning of dependencies](versioning.md#upper-version-pinning) by default, which is a **really bad idea**.[![](not-by-ai.svg){: .center}](https://notbyai.fyi)
